...
Весь проект — это **один интернациональный самораспаковывающийся файл** `_unpacker.bat`. Вам не нужно качать zip-архивы или клонировать репозиторий.
...
Билдер отслеживает наличие хуков. Если в папке `custom_files/ваш_профиль/` лежит файл **`hooks.sh`**, в меню загорится золотой индикатор **H**.

*   **Для чего:** Патчинг ядра, изменение DTS (например, для 16MB мода), Vermagic Hack.
*   **Vermagic Hack:** Скрипт сам скачает официальный манифест и подменит хэш ядра, чтобы вы могли ставить официальные `kmod-` пакеты без ошибок.
