...
*   **P (Packages)** — Наличие сторонних `.ipk/apk` пакетов.
...
*   **noarch/all:** Пакеты с универсальной архитектурой (скрипты, LuCI, конфиги) теперь корректно пропускаются для любых устройств.
*   **Автозапуск:** Срабатывает перед `docker compose up`, если в `custom_packages/` есть `.apk` файлы.
*   **Ручной запуск:** Кнопка **`[S] APK Scanner`** в главном меню — выбор профиля, сканирование, отчёт.

👉 **[Урок 8: Встраивание APK/IPK в Image Builder](docs/08-ib-apk-import-embed.md)** — пошаговое руководство, разбор реальных проблем и FAQ.
...
*   **Урок 8:** Встраивание APK/IPK в Image Builder (автосканер, noarch/all, валидация имён).
...
*   **Фишки:** **Atomic Downloads** (защита от сбоев сети), умный кэш (повторная сборка ~30 сек), интеграция сторонних `.ipk/apk` (папка `custom_packages`), **APK Scanner** (автоматическая валидация и переименование пакетов перед сборкой).
...
*   **Фишки:**
    ...
    *   **Binary-to-Source Import:** Позволяет встраивать готовые `.ipk/apk` пакеты (даже без исходников) прямо в прошивку, компилируемую из исходного кода.
...
*   `custom_packages/имя_профиля/` — Папка для `.ipk/apk` пакетов. Используется в **ImageBuilder** (прямое включение, автосканер валидации) и как источник для импорта в **SourceBuilder**.
*   `src_packages/имя_профиля/` — Для исходников. Используется для сторонних пакетов с GitHub и как **выходная папка** для `.ipk/apk`, обработанных импортером.
...
*   **`SRC_CORES="debug"`** — принудительная однопоточная сборка с максимальной детализацией (`make -j1 V=s`), идеальна для поиска и устранения неисправностей.
...
### ⚙️ Автоматический импорт `.ipk/apk` пакетов (Binary-to-Source)

Это одна из **ключевых возможностей** `Source Builder` v3.8+, решающая сложную проблему: как встроить в прошивку готовый `.ipk/apk` пакет, для которого у вас **нет исходного кода**.

**Проблема:**
Обычно для добавления пакета в `Source`-сборку требуется написать сложный `Makefile`. Если у вас есть только бинарный `.ipk/apk` (например, закрытый драйвер или старая программа), его интеграция превращается в кошмар: нужно вручную распаковывать архивы, прописывать пути, исправлять права доступа и надеяться, что система сборки не повредит готовые файлы.

**Решение в Билдере — полная автоматизация:**
Вместо ручной работы, вы просто используете встроенный импортер:

1.  **Кладете `.ipk/apk`** в папку `custom_packages/имя_вашего_профиля/`.
...
*   **Ошибка `Package not found`?**
    *   Убедитесь, что вы положили нужные `.ipk/apk` файлы в папку `custom_packages` или проверьте правильность написания имен пакетов в вашем `.conf` файле.
...
# checksum:MD5=56bac78fd4b935d626a1c5ec46732377