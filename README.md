...
Эта функция позволяет напрямую изменять исходный код OpenWrt перед компиляцией. Она работает по принципу **"зеркального оверлея"**: любой файл, который вы поместите в директорию `custom_patches/<ИМЯ_ПРОФИЛЯ>/`, перезапишет оригинальный файл в дереве исходников, сохраняя структуру путей.
