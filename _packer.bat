...
:: === 3. Добавляем функцию декодирования (С ПРОВЕРКОЙ НАЛИЧИЯ) ===
echo :DECODE_FILE>> "%NEW_UNPACKER_FILE%"
echo     :: Проверяем, существует ли файл>> "%NEW_UNPACKER_FILE%"
echo     if exist "%%~1" (>> "%NEW_UNPACKER_FILE%"
echo         echo [UNPACKER] [SKIP] Файл '%%~nx1' уже существует.>> "%NEW_UNPACKER_FILE%"
echo         exit /b>> "%NEW_UNPACKER_FILE%"
echo     )>> "%NEW_UNPACKER_FILE%"
echo.>> "%NEW_UNPACKER_FILE%"
...
echo.>> "%NEW_UNPACKER_FILE%"
echo     echo [UNPACKER] [WRITE] -^> %%~1>> "%NEW_UNPACKER_FILE%"
echo.>> "%NEW_UNPACKER_FILE%"
echo     :: Распаковка через PowerShell>> "%NEW_UNPACKER_FILE%"
