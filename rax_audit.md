...
---
## Анализ строк 1201-1400

**Ключевые моменты:**

*   **Инициализация `apcli`**: Замечена инициализация интерфейса `apclix0` (AP-Client), который предназначен для работы в режиме репитера/клиента. Он также проходит через аномальный цикл включения/выключения.
*   **Сканирование 5ГГц**: Несмотря на отключенный автовыбор канала, система запускает `ApSiteSurvey_by_wdev` - сканирование радиоэфира в диапазоне 5ГГц. В процессе сканирования основной интерфейс 5ГГц (`rax0`) временно отключается от моста `br-lan`.
*   **Подтверждение "хлопанья" для 5ГГц**: Лог окончательно подтверждает, что избыточный цикл перезагрузки свойственен всем Wi-Fi интерфейсам. После сканирования интерфейс `rax0` полностью выгружается (`VIRTUAL_INF_DOWN`, `mtk_ppe_dev_unregister_hook`) и затем инициализируется с нуля во второй раз.
*   **Повторная инициализация `rax0`**: Интерфейс `rax0` снова настраивается на канал 36 с шириной 160МГц, повторно регистрируется в аппаратном ускорителе и добавляется в мост `br-lan`.
*   **Повторная инициализация VAP и `apcli`**: Точно так же интерфейсы `wdev idx = 3` (виртуальная ТД) и `apclix0` проходят через второй цикл инициализации.

**Вывод по седьмому блоку:**
Этот сегмент лога демонстрирует системный характер избыточной и неэффективной инициализации Wi-Fi. Каждый сконфигурированный беспроводной интерфейс (основные ТД, виртуальные ТД, клиентские интерфейсы) проходит через полный цикл `init -> up -> scan/config -> down -> deinit -> re-init -> final up`. Хотя это и является значительным недостатком с точки зрения скорости загрузки и чистоты лога, система в итоге приходит в рабочее состояние, корректно настраивая все интерфейсы. На этом этапе все Wi-Fi интерфейсы запущены во второй и последний раз.
---
## Анализ строк 1401-1496 (Финал)

**Ключевые моменты:**

*   **Завершение сканирования 5ГГц**: Драйвер завершает повторное сканирование каналов 5ГГц, обнаружив 10 соседних сетей (`Total BSS[10]`), и возвращает радиомодуль на рабочий канал 36.
*   **Успешное подключение `apcli`**:
    *   Интерфейс `apclix0` (AP-Client) успешно подключается к родительской сети.
    *   `!!! LINK UP !!! wdev(name=apclix0, ...)`: Лог явно подтверждает установку соединения.
    *   `ssid=exeq, Channel=36`: Подтверждено имя родительской сети ("exeq") и канал.
    *   Далее следует успешный 4-полосный хендшейк WPA2 (`Receive msg 1... send Msg4`), что означает, что клиентское соединение установлено и защищено.
*   **Попытка запуска Docker**:
    *   `overlay: filesystem on /opt/docker/check-overlayfs-support... not supported as upperdir`: Эта строка появляется снова. Она указывает на то, что один из стартовых скриптов (вероятно, от пакета `docker-ce`) пытается проверить или создать `overlayfs` в нестандартной директории `/opt/docker` и терпит неудачу. Это является причиной, по которой Docker, скорее всего, не работает.
*   **Стабилизация системы**: Лог завершается периодическими проверками маячковых кадров (`BcnCheck`) и активностью `ppd_dev_setting`, что указывает на стабильную работу сетевой подсистемы.

---
### **Итоговое заключение по всему логу**

Загрузка роутера, собранного по профилю `rax3000m_emmc_test_new`, **проходит успешно**. Устройство достигает полностью рабочего состояния, все ключевые сетевые функции активированы.

**Сильные стороны конфигурации:**

1.  **Надежная основа**: Система корректно загружается с eMMC, использует файловую систему F2FS для `overlay`, что хорошо для накопителей этого типа.
2.  **Оптимизация производительности**: Присутствуют и корректно настроены все необходимые компоненты для высокой производительности: аппаратное ускорение NAT (`HNAT`, `PPE`) и Wi-Fi (`WED`, `WARP`, `bcn_offload`).
3.  **Оптимизация памяти**: Настроена многоуровневая система подкачки (быстрый ZRAM + медленный раздел на eMMC), что повышает стабильность при высоких нагрузках.
4.  **Корректная работа Wi-Fi**: Несмотря на сложности при запуске, оба радиомодуля (2.4ГГц и 5ГГц) в итоге запускаются на правильных каналах (автоматически выбранный для 2.4, фиксированный для 5), с высокой производительностью (160МГц для 5ГГц) и аппаратным ускорением.
5.  **Работа в режиме клиента**: Функция Wi-Fi клиента (`apcli`) полностью работоспособна и успешно подключается к вышестоящей точке доступа.

**Выявленные проблемы и особенности:**

1.  **Основная проблема: Избыточная инициализация Wi-Fi.** Самый заметный недостаток — это системная "болезнь" двойной инициализации всех беспроводных интерфейсов. Это не приводит к сбою, но значительно увеличивает время загрузки (в данном логе ~20-25 секунд тратится на повторный запуск Wi-Fi) и создает много "шума" в логе. Это, скорее всего, проблема взаимодействия скриптов `netifd` и проприетарных драйверов Mediatek.
2.  **Потенциальная проблема: Ошибка USB XHCI.** При инициализации внешнего SSD была зафиксирована ошибка `ERROR Transfer event for unknown stream`. Хотя диск и работает, это может быть источником проблем при интенсивных операциях чтения/записи.
3.  **Неработоспособность Docker.** Стартовый скрипт Docker не может подготовить `overlayfs` в директории `/opt/docker`. Это означает, что сам сервис Docker, скорее всего, не запустился и не функционирует. Это требует отдельной диагностики, если использование Docker является обязательным.

**Общий вывод:** Прошивка полностью работоспособна в качестве роутера. Основные функции выполняются корректно и с задействованием аппаратных ускорителей. Обнаруженные проблемы (медленный старт Wi-Fi, ошибка USB, нерабочий Docker) не являются критичными для основной функции устройства, но важны для дальнейшей доработки и отладки прошивки.
